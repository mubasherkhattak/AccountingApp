...
const GROUND_FLOOR = () => {
  ...
  // ✅ Default units
  const defaultUnits = Array.from({ length: 30 }, (_, i) => ({
    unitNo: "GF" + String(i + 1).padStart(2, "0"),
    area: [
      275, 329, 329, 199, 273, 612, 474, 752, 505, 485,
      479, 497, 470, 453, 453, 470, 193, 186, 443, 506,
      385, 385, 407, 401, 302, 461, 496, 518, 518, 387,
    ][i],
    date: "",
    downPayment: 0,
    total: 0,
  ...
  const loadUnits = () => {
    getFloorUnits("GroundFloor", (data) => {
      if (data.length > 0) {
        ...
        // Initialize with default units if collection is empty
        defaultUnits.forEach((u, index) => {
          addFloorUnit("GroundFloor", u, () => {
  ...
  const updateUnit = (id, field, value) => {
    const updated = units.map((u) => {
      if (u.id === id) {
        ...
        updateFloorUnit("GroundFloor", id, { [field]: value, total }, () => { });
  ...
  const saveUnitRecord = (unit) => {
    ...
    addUnitRecord(recordData, (success) => {
      if (success) {
        updateFloorUnit("GroundFloor", unit.id, { date: "", downPayment: 0, total: 0 }, () => {
          setUnits((prevUnits) =>
            prevUnits.map((u) =>
  ...
  const deleteUnit = (id) => {
    deleteFloorUnit("GroundFloor", id, () => {
  ...
  const addNewUnit = () => {
    ...
    const nextUnitNo = "GF" + String(lastNumber + 1).padStart(2, "0");
    ...
    addFloorUnit("GroundFloor", newUnitData, (newId) => {
      if (newId) {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.row}>
      ...
      <TextInput
        ...
        editable={parseInt(item.unitNo.replace(/[^0-9]/g, "")) > 30}
      />
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Ground Floor Units</Text>
